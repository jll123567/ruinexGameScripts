--THIS ENTIRE SCRIPT WAS NOT MY CREATION!!!
--that honor goes to Salvo_Starly
--I just modified this to work as its own game instead of runing in a scriptbuilder
--This is his user on roblox(i think):https://www.roblox.com/users/33104243/profile
--This is the raw version of the code I got:https://pastebin.com/raw/S298DJmF
--PM me at https://www.roblox.com/users/42465284/profile if I missed something in terms of credit and I'll fix it right away!
started= false
print("New instance creted: ",script.Name)
game.ReplicatedStorage.ruinexStart.OnServerEvent:Connect(function(player, mHit)--when the instance is ready and the player is ready
if not started then --prevent other players from restarting
started = true
print("ruinex started on player: \"",player.Name,"\"")
wait(.2)--init time
rainParts={}
plr=player
chr=plr.Character
chr.Humanoid.MaxHealth = 1000
chr.Humanoid.Health = 999
Head=chr.Head
...
function soundsCli(id,vol,pit)--send client sounds to client
game.ReplicatedStorage.ruinexSound:FireClient(plr,id,vol,pit)
end
function soundsServ(Par,id,vol,pit)--play server sounds
...
theme=create("Sound"){
Parent=plr,--made parent the player for easier removal on death
...
soundsServ(hit.Parent.Head,hs,hv,hp)
end
...
asdfz.CFrame=Root.CFrame
asdf2.CFrame=Root.CFrame*CFrame.new(0,30,0)
asdf.CFrame=CFrame.new(asdf.CFrame.p,mouseHit.p)
...
for i=1,200 do
swait()
asdf3.CFrame=CFrame.new(mouseHit.p)
asdfz.CFrame=Root.CFrame
asdf2.CFrame=Root.CFrame*CFrame.new(0,30,0)
asdf.CFrame=CFrame.new(asdf.CFrame.p,mouseHit.p)
asdfWeld.C0=clerp(asdfWeld.C0,asdfWeld.C0*CFrame.Angles(0,0,math.rad(4+hec)),.3)
...
soundsServ(ref,"161006093",1.2,1.2)
...
soundsServ(ref1,"206083232",1.2,1.5)
soundsServ(ref1,"206083252",1.2,1.3)
...
soundsServ(ref1,"206083252",1.2,1.6)
soundsServ(ref2,"206083252",1.2,1.6)
...
soundsServ(ref4,"206082273",3,.9)
soundsServ(ref4,"199145534",3,1)
...
soundsServ(ref2,"1724878544",1.3,1.2)
soundsServ(ref3,"1724878544",1.3,1.2)
...
soundsServ(ref1,"400765064",1,1.6)
soundsCli("2767090",1.2,1.2)
for i=0,1,0.03 do
swait()
local grab=FindNearestTorso(RArm.Position,4)
if grab~=nil and grabbed==nil then
grabbed=grab
soundsServ(grab,"200632875",1.2,1)
ref1.Transparency=1
ref2.Transparency=1
end
if grabbed~=nil then
...
soundsServ(ref1,"206082327",1.2,.9)
soundsServ(ref1,"199145534",1.1,1.3)
...
refz.Anchored=true
soundsServ(refz,"588732660",1.1,1)
...
soundsServ(refz,"1325061182",1.6,1.3)	
soundsServ(refz,"151776391",1.6,1)	
...
Lightning(e,RArm.Position, mouseHit.p, 15, 1, col[1], .8, 0, .02)

	
elseif choice==7 then
hum.WalkSpeed=0
soundsCli("588737452",1.3,.7)
...
local function lasr()
soundsCli("379557765",1.3,math.random(20,30)/10)
...
swait(80)
soundsServ(refsss,"588695708",1.3,.7)
...
doing=true
local sas=game.ReplicatedStorage.ruinexKey.OnServerEvent:Connect(function(player,k, mHit)
...
swait()
numz=numz+1
refe.CFrame=CFrame.new(mouseHit.p)*CFrame.Angles(math.rad(90),0,0)
if numz==10 then
numz=0
...
local dec=CFrame.new(ha,mouseHit.p)
...
if hit~=nil then
soundsServ(refz,"1489924400",.8,1.05)
...
refz.Anchored=true
game:GetService("Debris"):AddItem(refz,3)
soundsServ(refz,"151776391",1.4,1.5)
soundsServ(refz,"588705525",1.4,1)
...
soundsServ(ref,"161006033",1.2,1.1)
for i=1,10 do
...
for i=1,2 do
soundsCli("200632211",2,math.random(13,16)/20)
for i=0,1,0.12 do
swait()
...
soundsCli("1196176156",3,.5)
soundsCli("413503439",3,.5)
soundsCli("245520987",3,.5)
...
soundsCli("588698460",1.4,1.5)
soundsCli("588694531",1.4,1.3)
ok=true
for i=0,1,0.05 do
swait()
...
soundsServ(refz,"199146035",1.1,1)
local la=-6
for i=0,1,0.05 do
swait()
la=la+.5
...
ok=true
if choice==6 then
soundsCli("588717600",1.4,1.3)

elseif choice==8 then
soundsCli("588717937",1.4,1.2)
end
LockEffect(e,col[1],Root.CFrame,Root,40,40,40,2,2,2,.02,"Sphere",6,0,"")
for i=1,10 do
Effect(e,col[1],Root.CFrame*CFrame.Angles(math.rad(math.random(-360,360)),math.rad(math.random(-360,360)),math.rad(math.random(-360,360))),6,6,35,0,0,0,.03,"Sphere",4,.1,"",130,180,150)
...
game.ReplicatedStorage.ruinexClick.OnServerEvent:Connect(function(player, mHit)--handle clicks
if player.Name == plr.Name then
mouseHit = mHit
...
musicsetting=false
game.ReplicatedStorage.ruinexKey.OnServerEvent:Connect(function(player, k, mHit)--handle keypresses
if player.Name == plr.Name then
mouseHit = mHit
k=k:lower()
if attack==false then
...
vissi2()
--vissi3() --removed because it crashes on player death(not that it should matter)
...
--This script was dense with no useful comments when I found it.
--I barely understand it as it is
--if you're here to modify it ive noted the changes i made
--		(with the exception that all volumes related to "theme" have been turned down by 0.8)
--other than that I wish you luck
--Heres a count of the hours used on this script by me:18
--Hours by the creator(Salvo_Starly): who knows but likely alot...much respect.